...
export interface IndustryCategory {
  code: string;           // 大分類コード (A-T)
  name: string;           // 大分類名
  subCategories: {
    code: string;         // 中分類コード
    name: string;         // 中分類名
    keywords: string[];   // 検出用キーワード
    newsQuery: string;    // GNews検索クエリ（in=title用）
  }[];
}

// 主要な業種分類とニュース検索クエリ
export const INDUSTRY_CLASSIFICATIONS: IndustryCategory[] = [
  {
    code: 'A',
    name: '農業，林業',
    subCategories: [
 ...
 * キーワードから業種を検出する
 */
export function detectIndustryByKeywords(text: string): { 
  categoryCode: string;
  categoryName: string;
  ...
  for (const category of INDUSTRY_CLASSIFICATIONS) {
 ...
 * 業種名からニュース検索クエリを取得
 */
export function getNewsQueryByIndustryName(industryName: string): string {
