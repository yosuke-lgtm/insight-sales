...
export class CorporateService {
  ...
  private baseUrl = 'https://api.houjin-bangou.nta.go.jp/4/num';

  constructor() {
    this.appId = process.env.NATIONAL_TAX_API_KEY || '';
  }

  async searchCompany(name: string): Promise<CompanyProfile | null> {
    if (!this.appId) {
  ...
  private getDummyData(name: string): CompanyProfile {
    return {
      name: name,
   ...
   * Webサイトの内容から業種を推定
   */
  detectIndustryFromContent(bodyText: string, title: string): string {
    const text = `${title} ${bodyText}`.toLowerCase();
    
    // 業種キーワードマッピング（優先度順）
    // 各業種にニュース検索用のクエリも定義
    const industryKeywords: { industry: string; keywords: string[]; newsQuery: string }[] = [
      // BtoC サービス
   ...
   * 業種からニュース検索クエリを取得
   */
  getNewsQueryForIndustry(industryName: string): string {
    const industryNewsMap: { [key: string]: string } = {
      '美容・エステ': '美容サロン業界',
      '飲食店': '飲食業界',
