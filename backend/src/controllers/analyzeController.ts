...
import { EdinetService } from '../services/edinetService';
import { NewsService } from '../services/newsService';
import { GeminiService } from '../services/geminiService';
import { ScrapingService } from '../services/scrapingService';
import { CatrService } from '../services/catrService';
import { detectIndustryByKeywords } from '../data/industryClassification';
...
const catrService = new CatrService();
...
export const analyzeCompany = async (req: Request, res: Response) => {
  try {
    ...
    // 1. Identify Company
    const profile = await corporateService.searchCompany(companyName || domain);
    ...
    const scrapedData = targetUrl 
      ? await scrapingService.fetchPageContent(targetUrl) 
    ...
    if (scrapedData.bodyText) {
      ...
      quickAnalysis = await geminiService.detectIndustryAndQuickAnalysis(
        profile.name,
        scrapedData
      );
      ...
      if (isGenericResult) {
        ...
        const keywordResult = detectIndustryByKeywords(scrapedData.bodyText + ' ' + scrapedData.title);
    ...
    const [edinetFinancials, catrFinancials, pestleNews, additionalUrlData] = await Promise.all([
    ...
    let companyNews = await newsService.getCompanyNews(profile.name);
    if (companyNews.length === 0 && targetUrl) {
      console.log('News API returned 0 results for company, trying site scraping...');
      companyNews = await scrapingService.fetchCompanyNews(targetUrl);
    }
    ...
    const aiAnalysis = await geminiService.generateStrategyCarte(
      profile,
      financials,
      companyNews,
      allIndustryNews,
      ...
      inquiryBody,
      businessSegment,
      additionalUrlData // Pass scraped additional info (can be null)
    );
 ...
 */
export const analyzeInboundLead = async (req: Request, res: Response) => {
  try {
    ...
    const profile = await corporateService.searchCompany(searchKey);
    let targetUrl = '';
    ...
    const result = await geminiService.analyzeInboundLeadContext(
      companyName || profile?.name || '不明な企業',
      scrapedData, // { title, description, bodyText } compatible
      lpTitle || '不明なページ',
      lpUrl || '',
      inflowType || 'アクセス'
    );
