...
class GeminiService {
    constructor() {
        this.modelCandidates = ['gemini-2.5-flash-lite', 'gemini-2.0-flash-lite', 'gemini-2.0-flash'];
        // Vision/OCR用（マルチモーダル対応モデル）
        this.visionModelCandidates = ['gemini-2.5-flash-lite', 'gemini-2.0-flash-lite', 'gemini-2.0-flash'];
    ...
    isRetryableError(err) {
        const msg = typeof err === 'string' ? err : (err === null || err === void 0 ? void 0 : err.message) || '';
        return /429|rate limit|quota exceeded|503|unavailable|overloaded/i.test(msg);
    }
    // 429系で落ちたら次のモデルに切り替えるフォールバック呼び出し
    generateWithFallback(prompt_1) {
        return __awaiter(this, arguments, void 0, function* (prompt, useLiteOnly = false) {
            ...
            for (const modelName of candidates) {
                try {
                    ...
                    const res = yield model.generateContent(prompt);
                    return res.response.text();
                }
                catch (err) {
                    const retryable = this.isRetryableError(err);
                    console.warn(`Gemini call failed on ${modelName}:`, (err === null || err === void 0 ? void 0 : err.message) || err);
                    if (!retryable) {
                        throw err;
                    }
                    // リトライ可能なら次の候補へ
     ...
     */
    extractTextFromImages(images, hint) {
...
`;
            ...
            for (const modelName of this.visionModelCandidates) {
                try {
                    const model = this.genAI.getGenerativeModel({ model: modelName });
                    const res = yield model.generateContent({
                        contents: [
                            {
                                role: 'user',
                    ...
                    const retryable = this.isRetryableError(err);
                    console.warn(`Gemini OCR call failed on ${modelName}:`, (err === null || err === void 0 ? void 0 : err.message) || err);
                    if (!retryable) {
                        throw err;
                    }
                }
     ...
     */
    detectIndustryAndQuickAnalysis(companyName, scrapedData) {
        return __awaiter(this, void 0, void 0, function* () {
            const prompt = `
...
`;
            try {
                const text = yield this.generateWithFallback(prompt, true);
                ...
                if (jsonMatch) {
                    ...
                    if (!normalizedQueries.regulation)
                        normalizedQueries.regulation = `${industryName} 法規制`;
                    if (!normalizedQueries.clientMarket)
                        normalizedQueries.clientMarket = `${industryName} 市場`;
                    if (!normalizedQueries.technology)
                        normalizedQueries.technology = `${industryName} DX`;
                    if (!normalizedQueries.industry)
                        normalizedQueries.industry = `${industryName} 業界`;
                    return {
    ...
    generateStrategyCarte(company, financials, companyNews, industryNews, scrapedData, inquiryBody, businessSegment, // 対象事業（複数事業がある場合）
    additionalUrlData // 追加参考URLの情報
    ) {
        return __awaiter(this, void 0, void 0, function* () {
...
- 採用ページリンク数: ${scrapedData.recruitLinks.length}
- スクレイピングされた企業情報:
  - 売上高: ${((_a = scrapedData.companyInfo) === null || _a === void 0 ? void 0 : _a.revenue) || '不明'}
  - 資本金: ${((_b = scrapedData.companyInfo) === null || _b === void 0 ? void 0 : _b.capital) || '不明'}
  - 従業員数: ${((_c = scrapedData.companyInfo) === null || _c === void 0 ? void 0 : _c.employees) || '不明'}
  - 設立: ${((_d = scrapedData.companyInfo) === null || _d === void 0 ? void 0 : _d.founded) || '不明'}
  - 決算: ${((_e = scrapedData.companyInfo) === null || _e === void 0 ? void 0 : _e.fiscalYearEnd) || '不明'}
...
\`\`\`json
{
  ...
  "financialHealth": {
    "status": "財務状況と規模感。スクレイピングされた従業員数（${((_f = scrapedData.companyInfo) === null || _f === void 0 ? void 0 : _f.employees) || '不明'}）や売上高（${((_g = scrapedData.companyInfo) === null || _g === void 0 ? void 0 : _g.revenue) || '不明'}）を必ず明記すること。例: '従業員数1,950名の大規模企業で、売上高も安定している'",
    "concern": "財務上の懸念点（あれば）",
    "investmentCapacity": "投資余力（高/中/低）。利益状況や事業の安定性から推定",
    "budgetCycle": "予算決裁の時期（一般的な日本企業なら3月、外資なら12月など、決算月: ${((_h = scrapedData.companyInfo) === null || _h === void 0 ? void 0 : _h.fiscalYearEnd) || '不明'} から推測）",
...
`;
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    ...
                    if (jsonMatch && jsonMatch[1]) {
                        ...
                        return yield this.ensureConclusions(parsed, company);
                    }
    ...
    getMissingConclusionPaths(result) {
        const required = [
            ['pestle', 'conclusion'],
        ...
        for (const path of required) {
            ...
            for (const key of path) {
                cur = cur === null || cur === void 0 ? void 0 : cur[key];
            }
            if (isMissing(cur))
    ...
    ensureConclusions(result, company) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                for (const path of missing) {
                    ...
                    if (sections[sectionKey])
                        continue;
                    sections[sectionKey] = result === null || result === void 0 ? void 0 : result[sectionKey];
                }
                const repairPrompt = `
...
`;
                const text = yield this.generateWithFallback(repairPrompt, true);
     ...
     */
    analyzeInboundLeadContext(companyName, scrapedData, lpTitle, lpUrl, inflowType // '資料DL' | 'お問い合わせ' etc.
    ) {
        return __awaiter(this, void 0, void 0, function* () {
            const prompt = `
